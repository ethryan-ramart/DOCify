  ...
  return (
    <Card key={doc.id}>
      <CardHeader className="relative">
        <FavButton className="absolute top-4 right-4 text-2xl" isFav={isFav} bookUuid={doc.id} toggleFavorite={toggleFavorite} />
      ...
      <CardContent >
        <div className="flex flex-col justify-center items-center space-y-2">
          ...
          <img src={doc.cover_url} alt={`${doc.title}-cover`} className="max-h-40 aspect-[9/16]" />
          <p className="line-clamp-1 min-h-4">{doc.description}</p>
        </div>
      </CardContent>
      <CardFooter >
        <div className="flex space-x-2 w-full">
          <ViewExploreDocument id={doc.id} className="flex-grow" />
          <DownloadDocument publicUrl={doc.file_url} className="flex-grow" />
        </div>
      </CardFooter>
    </Card >
  );
